...
while(True):
    ...
    if (time.time()-prevTime > 300):
        ...
        for n in remoteNodes:
            ...
            with open('/home/lde/Share/'+n+'/'+str(datetime.now().year)+'/'+str(datetime.now().month)+'/'+filename+'.txt','a') as f:
        ...
        if xbee_message != None:
            ...
            for n in labID:
                if remoteID==n:
                    #print("El nodo remoto es %s y corresponde al %s" %(remoteID,labName[labID.index(n)]))
                    remoteID = labName[labID.index(n)]
